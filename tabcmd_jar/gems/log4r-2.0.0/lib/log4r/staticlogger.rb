  ...
  class Logger
    ...
    class NDC
      ...
      def self.set(key, value)
        (Thread.current[:log4r_NDC_pairs] ||= {})[key] = value
      end

      def self.get(*args)
        if (args.nil?)       ## something odd inside outputter calls this at init time with no args, just smile and keep walking

          return ""
        else
          (Thread.current[:log4r_NDC_pairs] ||= {})[args[0]] || ""
        end
      end
      ...
      def self.dotted()
        (Thread.current[:log4r_NDC_key] || []).join('.')
      end

      def self.clear_for_test()
        Thread.current[:log4r_NDC_key] = []
        Thread.current[:log4r_NDC_pairs] = {}
      end
